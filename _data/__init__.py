...
# Per-aperture mean-wave elevation series (produced by
# tools/spectra_vs_aperture.py --series-out). Used by the walkthrough notebook
# to draw the aperture spectral comparison without re-running the reduction.
APERTURE_ELEVATION_FILENAME = "asit2019_aperture_elevation_60s.nc"
...
def aperture_elevation_path() -> Path:
    """Local path to the per-aperture mean-wave elevation artifact.

    Produced by `tools/spectra_vs_aperture.py` (its --series-out default writes
    here). Holds the full-frame / 0.5x / 0.25x aperture elevation series at the
    native frame rate; the walkthrough notebook reads it to draw the aperture
    spectral comparison. Unlike the other example files this one is NOT
    necessarily committed -- it is regenerated by running the spectra tool over
    the full stack. Callers should handle its absence gracefully.
    """
    path = EXAMPLES_DIR / APERTURE_ELEVATION_FILENAME
    if not path.exists():
        raise FileNotFoundError(
            f"{APERTURE_ELEVATION_FILENAME} not found in {EXAMPLES_DIR}. It is "
            f"produced by running the spectra tool over the full stack:\n"
            f"    python tools/spectra_vs_aperture.py --reduce-downsample 4\n"
            f"(writes there by default)."
        )
    return path
